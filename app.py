...
from db import register_user, authenticate_user, save_query_to_history, get_query_history  # Giả sử các hàm này đã được định nghĩa trong db.py
import threading
...
def login():
    ...
    if user:
        return jsonify({"user_id": user[0], "message": f"Chào mừng {username}!"}), 200
...
@app.route('/api/get_query_history/<int:user_id>', methods=['GET'])
def get_query_history_api(user_id):
    history = get_query_history(user_id)  # Gọi hàm lấy lịch sử truy vấn
    return jsonify(history)  # Trả về dữ liệu dưới dạng JSON
